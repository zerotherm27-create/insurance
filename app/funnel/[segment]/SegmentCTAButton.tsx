...
export function SegmentCTAButton({ segment, cta }: Props) {
  ...
  return (
    <button
      onClick={handleStart}
      disabled={loading}
      className="inline-flex items-center justify-center w-full px-8 py-4 text-base rounded-xl font-sans font-semibold tracking-wide bg-gold text-navy-dark hover:bg-gold-soft shadow-lg hover:-translate-y-0.5 transition-[background-color,transform] duration-200 focus:outline-none focus-visible:ring-2 focus-visible:ring-gold/60 disabled:opacity-70 disabled:cursor-wait min-h-[52px]"
    >
      {loading ? (
